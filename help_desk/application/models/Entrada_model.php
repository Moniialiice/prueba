...
class Entrada_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT e.id_oficioEntrada, e.no_oficioEntrada, e.firma_origen, e.cargo, e.peticion, e.arch_entrada, e.fecha_ent, e.fecha_rec, e.fecha_real, u.nombre, u.apellidop, u.apellidom FROM oficio_entrada as e, usuario as u WHERE e.no_oficioEntrada LIKE '%$search%' AND e.fecha_rec BETWEEN STR_TO_DATE(REPLACE('$date1','/','-'), GET_FORMAT(date,'ISO')) AND STR_TO_DATE(REPLACE('$date2','/','-'), GET_FORMAT(date, 'ISO')) AND e.atencion = u.id_usuario ORDER BY e.fecha_rec DESC");
        return $query->result();
    }
