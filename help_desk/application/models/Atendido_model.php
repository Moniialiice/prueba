...
class Atendido_model extends CI_model
{
    ...
    {
        $query = $this->db->query("SELECT a.id_oficioAtendido, a.fecha_atendido, a.nombre_aten, a.cargo_aten, a.descripcion, a.arch_atendido, a.copia_a, u.id_usuario, u.nombre, u.apellidop, u.apellidom, s.id_oficioseg, s.nomenclatura FROM oficio_atendido as a, usuario as u, oficio_seguimiento as s WHERE s.nomenclatura LIKE '%$search%' AND a.fecha_atendido BETWEEN STR_TO_DATE(REPLACE('$fecha1','/','-'), GET_FORMAT(date,'ISO')) AND STR_TO_DATE(REPLACE('$fecha2','/','-'), GET_FORMAT(date,'ISO')) AND u.id_usuario = a.atencion AND s.id_oficioseg = a.id_oficioseg");
        return $query->result();
    }
